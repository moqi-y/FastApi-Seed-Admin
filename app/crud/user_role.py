...
# 根据用户id获取角色名称
async def get_role_name_by_user_id(user_id: int):
    try:
        query = select(UserRole).where(UserRole.user_id == user_id)
        result = session.exec(query).all()
        role_names = []
        if result:
            for role in result:
                roles = await get_role_by_id(role.role_id)
                role_names.append(roles.name)
            return role_names
    except Exception as e:
        print("get_role_name_by_user_id() SQL Error: ", e)
        return None
    finally:
        session.close()
...
async def update_user_role(user_role: UserRoleUpdate):
    ...
    try:
        query = select(UserRole).where(UserRole.user_id == user_role.user_id, UserRole.role_id == user_role.role_id)
        res_user_role = session.exec(query).first()
        if res_user_role:
            res_user_role.user_id = user_role.user_id
            res_user_role.role_id = user_role.role_id
            session.commit()
            session.refresh(res_user_role)
            return True
        else:
            await add_user_role(UserRoleCreate(user_id=user_role.user_id, role_id=user_role.role_id))
            return True
    except Exception as e:
        print("update_user_role() SQL Error: ", e)
        return None
    finally:
        session.close()
